...
function PlayState:render()
    love.graphics.push()
    love.graphics.draw(textures['backgrounds'], frames['backgrounds'][self.background], math.floor(-self.backgroundX), 0)
    love.graphics.draw(
    ...
    love.graphics.draw(textures['backgrounds'], frames['backgrounds'][self.background], math.floor(-self.backgroundX + 256), 0)
    love.graphics.draw(
        textures['backgrounds'],
        frames['backgrounds'][self.background],
...
function PlayState:updateCamera()
    ...
    self.camX = math.max(
        0,
        math.min(
            (TILE_SIZE * self.tileMap.width) - VIRTUAL_WIDTH,
...
function PlayState:spawnEnemies()
    ...
    for x = 1, self.tileMap.width do
        local groundFound = false -- flag for whether there's ground on this column of the level
