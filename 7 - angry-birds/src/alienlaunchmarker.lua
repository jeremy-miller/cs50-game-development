...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        elseif love.mouse.wasReleased(1) and self.aiming then
            ...
            self.alien.fixture:setRestitution(0.4) -- bounciness of alien
            self.alien.body:setAngularDamping(1) -- add friction when alien is rolling on ground
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        love.graphics.draw(textures['aliens'], frames['aliens'][9], self.shiftedX - 17.5, self.shiftedY - 17.5)
