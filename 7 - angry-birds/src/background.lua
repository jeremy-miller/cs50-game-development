...
function Background:init()
    ...
    self.width = textures[self.background]:getWidth()
    self.xOffset = 0
end
...
function Background:render()
    love.graphics.draw(textures[self.background], self.xOffset, -128)
    love.graphics.draw(textures[self.background], self.xOffset + self.width, -128)
    love.graphics.draw(textures[self.background], self.xOffset - self.width, -128)
end
